...
and we have a device" - she attaches something to your wrist - 
"that will let you fix the changes with no such propagation delay. It's configured to send you 500 years further 
into the past every few days; that was the best we could do on such short notice."

"The bad news is that we are detecting roughly fifty anomalies throughout time; 
the device will indicate fixed anomalies with stars. The other bad news is that we only have one device and 
you're the best person for the job! Good lu--" She taps a button on the device and you suddenly feel like you're falling. 
...
const findFrequencyDuplicate = (list: Array<number>) => {
  ...
  do {
    for (let freq of list) {
      frequency += freq;
      if (frequencyTrack.indexOf(frequency) !== -1) {
        repeat = true;
        return frequency;
      }
      frequencyTrack.push(frequency);
    }
  } while (repeat === false);
}

findFrequencyDuplicate(list);
// 82516
